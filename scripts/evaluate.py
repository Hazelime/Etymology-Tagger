...
def main() -> None:
    ...
    metrics = {
        ...
        "source_pair_logistic": precision_recall_f1(
            dataset.y_pairs[test_indices],
            predict_scores(dataset.x[test_indices], model["pair_logistic"][0], model["pair_logistic"][1]),
            _threshold_array(thresholds.get("source_pair_logistic"), config["threshold"]),
        ),
        "source_pair_centroid": precision_recall_f1(
            dataset.y_pairs[test_indices],
            predict_centroid_scores(dataset.x[test_indices], model["pair_centroid"]),
            _threshold_array(thresholds.get("source_pair_centroid"), config["threshold"]),
        ),
